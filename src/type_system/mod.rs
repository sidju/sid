    ...
    match self {
      ...
      SidType::Literal(lit) => match lit.as_ref() {
        ...
        DataValue::List(pat_items) => match value {
          DataValue::List(val_items) =>
            ...
            && pat_items.iter().zip(val_items).all(|(p, v)| {
              SidType::Literal(Box::new(p.clone())).matches(v)
            }),
          _ => false,
        },
        // Set literal → enum / union: value must match at least one element.
        DataValue::Set(pat_items) => pat_items.iter().any(|p| {
          SidType::Literal(Box::new(p.clone())).matches(value)
        }),
        ...
        DataValue::Map(pat_entries) => {
          let all_labels = pat_entries.iter().all(|(k, _)| matches!(k, DataValue::Label(_)));
          if all_labels {
            // Struct / ordered named-field pattern.
            match value {
              DataValue::Map(val_entries) => {
                let val_label_entries: Vec<_> = val_entries.iter()
                  .filter(|(k, _)| matches!(k, DataValue::Label(_)))
                  .collect();
                pat_entries.len() == val_label_entries.len()
                && pat_entries.iter().zip(val_label_entries).all(|((pk, pv), (vk, vv))| {
                  pk == vk && SidType::Literal(Box::new(pv.clone())).matches(vv)
                })
              },
            ...
            match value {
              DataValue::Map(val_entries) => pat_entries.iter().all(|(pk, pv)| {
                val_entries.iter()
                  .find(|(vk, _)| vk == pk)
                  .map_or(false, |(_, vv)| SidType::Literal(Box::new(pv.clone())).matches(vv))
              }),
              _ => false,
            }
          }
      ...
      SidType::Fn { args, ret } => match value {
        ...
        DataValue::Script   { args: val_args, ret: val_ret, .. } => {
          ...
          let val_arg_types: Option<Vec<SidType>> = val_args.as_ref()
...
mod tests {
  ...
  #[test] fn literal_list_is_tuple_type() {
    ...
    assert!(!ty.matches(&DataValue::List(vec![DataValue::Bool(true), DataValue::Int(1)])));
  }

  #[test] fn literal_map_label_keys_is_ordered_structural() {
    // All-label-key Map pattern: strict ordered match (same count, same order).
    let ty = SidType::Literal(Box::new(DataValue::Map(vec![
      (DataValue::Label("x".to_owned()), DataValue::Type(SidType::Int)),
    ])));
  ...
  #[test] fn literal_map_non_label_keys_is_structural() {
    // Non-label-key Map pattern: structural match (pattern entries must be present, extras ok).
    let ty = SidType::Literal(Box::new(DataValue::Map(vec![
      (DataValue::Int(1), DataValue::Type(SidType::Bool)),
    ])));
    assert!( ty.matches(&DataValue::Map(vec![
  ...
  #[test] fn union_of_literals_matches_exact_values() {
    let u = SidType::Literal(Box::new(DataValue::Set(vec![
      DataValue::Int(1),
      DataValue::Int(2),
    ])));
  ...
  fn struct_type(fields: &[(&str, SidType)]) -> SidType {
    SidType::Literal(Box::new(DataValue::Map(
      fields.iter().map(|(n, t)| (
        DataValue::Label(n.to_string()),
        DataValue::Type(t.clone()),
      )).collect()
    )))
  }
  ...
  fn struct_val(fields: &[(&str, DataValue)]) -> DataValue {
    DataValue::Map(
      fields.iter().map(|(n, v)| (DataValue::Label(n.to_string()), v.clone())).collect()
    )
  }

  #[test] fn struct_matches_exact_fields_in_order() {
    let ty = struct_type(&[("x", SidType::Int), ("y", SidType::Bool)]);
    assert!( ty.matches(&struct_val(&[("x", DataValue::Int(1)), ("y", DataValue::Bool(true))])));
  ...
  #[test] fn struct_matches_list_positionally() {
    let ty = struct_type(&[("x", SidType::Int), ("y", SidType::Bool)]);
    assert!( ty.matches(&DataValue::List(vec![DataValue::Int(1), DataValue::Bool(true)])));
    assert!(!ty.matches(&DataValue::List(vec![DataValue::Int(1)])));
    assert!(!ty.matches(&DataValue::List(vec![DataValue::Bool(true), DataValue::Int(1)])));
