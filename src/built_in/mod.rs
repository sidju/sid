...
fn pop_arg(
    ...
    use crate::{ProgramValue, TemplateValue};
    match data_stack.pop() {
        Some(TemplateValue::Literal(ProgramValue::Data(v))) => Ok(v),
        Some(other) => anyhow::bail!(
            "{}: argument is not a concrete value: {:?}",
            builtin_name,
            other
        ),
        None => anyhow::bail!(
            "{}: expected an argument but the stack was empty",
            builtin_name
        ),
    }
}
    ...
    match arg {
        ...
        DataValue::List(mut items) if items.len() == 2 => {
            let path = match items.remove(0) {
                ...
                other => anyhow::bail!(
                    "c_load_header: first list element must be Str (path), got {:?}",
                    other
                ),
            };
            let lib_name = match items.remove(0) {
                DataValue::Str(s) => cstring_to_string(s),
                other => anyhow::bail!(
                    "c_load_header: second list element must be Str (lib_name), got {:?}",
                    other
                ),
            };
            Ok((path, lib_name))
        ...
        other => anyhow::bail!(
            "c_load_header expects Str(path) or [Str(path), Str(lib_name)], got {:?}",
            other
        ),
    }
}
...
fn stem_of(path: &str) -> anyhow::Result<String> {
    std::path::Path::new(path)
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "c_load_header: could not derive lib_name from path '{}'",
                path
            )
        })
}
    ...
    match arg {
        ...
        DataValue::List(mut items) if items.len() == 2 => {
            let path = match items.remove(0) {
                DataValue::Str(s) => cstring_to_string(s),
                other => anyhow::bail!(
                    "c_link_lib: first list element must be Str (lib_path), got {:?}",
                    other
                ),
            };
            let name = match items.remove(0) {
                DataValue::Str(s) => cstring_to_string(s),
                other => anyhow::bail!(
                    "c_link_lib: second list element must be Str (lib_name), got {:?}",
                    other
                ),
            };
            Ok((path, name))
        ...
        other => anyhow::bail!(
            "c_link_lib expects Str(lib_path) or [Str(lib_path), Str(lib_name)], got {:?}",
            other
        ),
    }
}
...
impl InterpretBuiltIn for LoadScope {
    fn execute(
        ...
        let entries = match pop_arg_resolved(
            data_stack,
            "load_scope",
            local_scope,
            global_state.scope,
...
impl InterpretBuiltIn for PtrCast {
    fn execute(
        ...
        let pointee_ty = match new_type {
            ...
            DataValue::Label(name) => match get_from_scope(
                &name,
                Some(local_scope),
                ...
                DataValue::Type(ty) => ty,
                other => anyhow::bail!(
                    "ptr_cast: label '{}' resolves to {:?}, not a Type",
                    name,
                    other
                ),
            },
            other => anyhow::bail!(
                "ptr_cast: type argument must be a Type or label, got {:?}",
                other
            ),
        };
...
impl InterpretBuiltIn for DebugStack {
    fn execute(
        ...
        let n = match pop_arg_resolved(
            ...
            DataValue::Int(n) => {
                anyhow::bail!("debug_stack: count must be non-negative, got {}", n)
            }
...
impl InterpretBuiltIn for WhileDo {
    fn execute(
        ...
        program_stack.push(crate::ProgramValue::CondLoopStart {
            cond: cond_val.clone(),
            body: body_val,
        });
...
impl InterpretBuiltIn for DoWhile {
    fn execute(
        ...
        program_stack.push(crate::ProgramValue::CondLoop {
            cond: cond_val.clone(),
            body: body_val.clone(),
            expected_len,
        });
...
impl InterpretBuiltIn for FnType {
    fn execute(
        ...
        Ok(vec![DataValue::Type(SidType::Fn {
            args: None,
            ret: None,
        })])
    }
...
impl InterpretBuiltIn for TypedArgs {
    fn execute(
        ...
        let target_val = pop_arg_resolved(
            data_stack,
            "typed_args",
            local_scope,
            global_state.scope,
        ...
        let types_val = pop_arg_resolved(
            data_stack,
            "typed_args",
            local_scope,
            global_state.scope,
        ...
        match target_val {
            DataValue::Substack { body, ret, .. } => Ok(vec![DataValue::Substack {
                body,
                args: Some(named_args),
                ret,
            }]),
            DataValue::Script { body, ret, .. } => Ok(vec![DataValue::Script {
                body,
                args: Some(named_args),
                ret,
            }]),
            DataValue::Type(SidType::Fn { ret, .. }) => Ok(vec![DataValue::Type(SidType::Fn {
                args: Some(type_only),
                ret,
            })]),
            other => anyhow::bail!(
...
impl InterpretBuiltIn for TypedRets {
    fn execute(
        ...
        let types: Vec<SidType> = list_to_type_vec(types_val, "typed_rets")?
            ...
            .collect();
        match target_val {
            DataValue::Substack { body, args, .. } => Ok(vec![DataValue::Substack {
                body,
                args,
                ret: Some(types),
            }]),
            DataValue::Script { body, args, .. } => Ok(vec![DataValue::Script {
                body,
                args,
                ret: Some(types),
            }]),
            DataValue::Type(SidType::Fn { args, .. }) => Ok(vec![DataValue::Type(SidType::Fn {
                args,
                ret: Some(types),
            })]),
            other => anyhow::bail!(
