...
fn parse_substack() {
    ParseTestFixture {
        input: "(\"data\" 5 $1)",
        expected_output: vec![Template::substack((
            vec![
...
fn parse_script() {
    ParseTestFixture {
        input: "<\"hi\" 5>",
        expected_output: vec![Template::script((
            vec![
...
fn parse_list() {
    ParseTestFixture {
        input: "[\"data\" 5 $1]",
        expected_output: vec![Template::list((
            vec![
                DataValue::Str(std::ffi::CString::new("data").unwrap()).into(),
                DataValue::Int(5).into(),
                TemplateValue::ParentStackMove(1),
...
fn parse_set() {
    ParseTestFixture {
        input: "{1, 2, 3}",
        expected_output: vec![Template::set((
            vec![
...
fn parse_map() {
    ParseTestFixture {
        input: "{x: 1, y: 2}",
        expected_output: vec![Template::map(
            vec![
                (
