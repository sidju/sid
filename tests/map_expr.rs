...
fn single_token_map_value_unchanged() {
  ...
  assert_eq!(stack, vec![DataValue::Map(vec![
    (DataValue::Label("x".to_owned()), DataValue::Int(42)),
  ])]);
}
...
fn multi_token_value_via_local_binding() {
  ...
  let bound = run_snippet(
    "result_type types.str ptr ! local! {result: $result_type}"
  );
  let inline = run_snippet("{result: types.str ptr !}");
  assert_eq!(bound, inline, "inline multi-token expr should match pre-bound local");
}
...
fn multi_token_value_invoke_builtin() {
  ...
  assert_eq!(stack, vec![DataValue::Map(vec![
    (DataValue::Label("x".to_owned()), DataValue::Type(SidType::Pointer(Box::new(SidType::Str)))),
  ])]);
}
...
fn multi_token_map_mixed_inline_and_literal() {
  ...
  assert_eq!(stack, vec![DataValue::Map(vec![
    (DataValue::Label("a".to_owned()), DataValue::Type(SidType::Int)),
    (DataValue::Label("b".to_owned()), DataValue::Type(SidType::Pointer(Box::new(SidType::Str)))),
  ])]);
}
...
fn multi_token_key_type_expr() {
  ...
  assert_eq!(stack, vec![DataValue::Map(vec![
    (DataValue::Type(SidType::Pointer(Box::new(SidType::Str))), DataValue::Int(42)),
  ])]);
}
