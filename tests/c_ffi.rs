...
fn c_load_header_str_arg_derives_lib_name() {
    ...
    let mut result = builtins["c_load_header"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Str(std::ffi::CString::new(fixture_header()).unwrap())))], &mut state)
...
fn c_load_header_list_arg_uses_explicit_lib_name() {
    ...
    let mut state = GlobalState::new(&mut scope);
    let mut result = builtins["c_load_header"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(arg))], &mut state)
        .expect("c_load_header failed");
...
fn c_load_header_error_on_wrong_arg() {
    ...
    let result = builtins["c_load_header"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Int(42)))], &mut state);
...
fn c_link_lib_preloads_library() {
    ...
    let mut state = GlobalState::new(&mut scope);
    builtins["c_link_lib"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Str(std::ffi::CString::new(TEST_LIB).unwrap())))], &mut state)
...
fn c_link_lib_error_on_missing_library() {
    ...
    let result = builtins["c_link_lib"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Str(std::ffi::CString::new("/nonexistent/lib.so").unwrap())))], &mut state);
    assert!(result.is_err());
}
...
fn drop_discards_value() {
    ...
    let result = builtins["drop"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Int(99)))], &mut state)
        .expect("drop failed");
...
fn assert_passes_on_true() {
    ...
    let result = builtins["assert"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Bool(true)))], &mut state)
...
fn assert_errors_on_false() {
    ...
    let mut scope = HashMap::new();
    let mut state = GlobalState::new(&mut scope);
    let result = builtins["assert"].execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Bool(false)))], &mut state);
    assert!(result.is_err());
}
...
fn assert_error_on_non_bool() {
    ...
    let mut scope = HashMap::new();
    let mut state = GlobalState::new(&mut scope);
    let result = builtins["assert"].execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Int(1)))], &mut state);
    assert!(result.is_err());
}
...
fn load_scope_error_on_non_struct() {
    ...
    let mut state = GlobalState::new(&mut scope);
    let result = builtins["load_scope"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Int(1)))], &mut state);
    assert!(result.is_err());
}
...
fn clone_duplicates_value() {
    ...
    let result = builtins["clone"]
        .execute(&mut vec![sid::TemplateValue::Literal(sid::ProgramValue::Data(DataValue::Int(7)))], &mut state)
        .expect("clone failed");
...
fn clone_error_on_no_value() {
    ...
    let mut scope = HashMap::new();
    let mut state = GlobalState::new(&mut scope);
    let result = builtins["clone"].execute(&mut vec![], &mut state);
    assert!(result.is_err());
}
