...
pub fn parse_test_fixture(
    ...
    assert_eq!(
        expected_output_template, output.0,
        "Parsed template didn't match expectations"
    );
    assert_eq!(
        expected_output_stack_entries_consumed, output.1,
...
fn parse_bool() {
    parse_test_fixture(
        "true false",
        vec![DataValue::Bool(true).into(), DataValue::Bool(false).into()],
        0,
    )
}
...
fn parse_integer() {
    parse_test_fixture(
        "-10 500000",
        vec![DataValue::Int(-10).into(), DataValue::Int(500000).into()],
        0,
    )
}
...
fn parse_with_stack_template() {
    parse_test_fixture("$3", vec![TemplateValue::ParentStackMove(3)], 3)
}
...
fn parse_with_parent_label() {
    parse_test_fixture(
        "$label_name",
        vec![TemplateValue::ParentLabel("label_name".to_owned())],
        0,
    )
}
